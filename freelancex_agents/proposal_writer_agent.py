...
try:
    from freelancex_agents import Agent, Runner, Session, tool
except ImportError:
    # Handle import error gracefully
    Agent = None
    Runner = None
    Session = None
    tool = lambda func: func  # Fallback decorator
...
class ProposalWriterAgent(BaseAgent):
    ...
    def _get_proposal_generation_tool(self):
        ...
        def generate_proposal(job_description: str, user_profile: str) -> str:
            ...
            try:
                ...
                budget_mentioned = "budget" in job_description.lower()
                
                proposal = f"""
                Dear Client,
                
                I'm excited about your {project_type} project. Based on your description:
                "{job_description[:100]}..."
                
                I have extensive experience in this area and can deliver high-quality results.
                
                Best regards,
                FreelanceX.AI Assistant
                """
    ...
    def _get_pricing_strategy_tool(self):
        ...
        def suggest_pricing(project_scope: str, complexity: str) -> str:
            ...
            try:
                ...
                suggested_rate = base_rates.get(complexity.lower(), 3000)
                
                strategy = f"""
                Pricing Strategy for {complexity} project:
                
                Recommended Rate: ${suggested_rate:,}
                
                Factors considered:
                - Project complexity: {complexity}
                - Scope: {project_scope[:100]}...
                - Market rates for similar projects
                
                Negotiation range: ${int(suggested_rate * 0.8):,} - ${int(suggested_rate * 1.2):,}
                """
                return strategy.strip()
            except Exception as e:
                return f"Error suggesting pricing: {str(e)}"
    ...
    async def process_custom_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        """Process proposal writing tasks with custom logic (fallback method)"""
        try:
            return await self.process_task(task)
        except Exception as e:
            logger.error(f"❌ Proposal writer agent custom task processing failed: {str(e)}")
            return {
                'success': False,
                'error': str(e),
...
{your_name}
    ...
    def _get_handoff_agents(self) -> List:
        """Get handoff agents for proposal writer"""
