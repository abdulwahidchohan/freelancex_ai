...
try:
    ...
    _OPENAI_CLIENT: Optional[OpenAI] = OpenAI() if os.getenv("OPENAI_API_KEY") else None
except Exception:
    _OPENAI_CLIENT = None
...
def analyze_request(user_input: str) -> RequestAnalysis:
    ...
    # Try model-driven classification first
    if _OPENAI_CLIENT:
        try:
            system = (
                "Classify the user's request for a freelancer assistant. "
                "Return strict JSON with fields: primary_intent (string agent name), "
                "required_agents (array of agent names), complexity (low|medium|high), reasoning (short). "
                "Valid agent names: Executive Core Agent, Cognitive Core Agent, Job Search Agent, Proposal Writer Agent, "
                "Web Research Agent, Math Agent, Marketing Agent, Client Liaison Agent, Negotiator Agent, Automation Agent, "
                "User Experience Agent, Security Agent, Expansion Agent."
            )
            prompt = f"User: {user_input}\nReturn JSON only."
            resp = _OPENAI_CLIENT.chat.completions.create(
                model=os.getenv("FREELANCEX_MODEL", "gpt-4o-mini"),
            ...
            return RequestAnalysis(
                primary_intent=parsed.get("primary_intent", "Cognitive Core Agent"),
                confidence=0.75,
                required_agents=parsed.get("required_agents", [parsed.get("primary_intent", "Cognitive Core Agent")]),
                complexity=parsed.get("complexity", "medium"),
                reasoning=parsed.get("reasoning", "Model classified intent"),
            )
        except Exception:
            # Fall back to keyword routing below
            pass
